...
function main() {
  ...
  const versionLines = [
    ...
    "trace_script = ampcode/scripts/trace-amp-runtime.cjs",
    "tool_script = ampcode/scripts/extract-amp-tools.cjs",
    "capture = tmux interactive session with `BUN_OPTIONS=--preload ampcode/scripts/trace-amp-runtime.cjs amp --dangerously-allow-all --no-ide --no-notifications` plus `amp tools show --json` for each mode",
    `modes = ${modes.join(", ")}`,
